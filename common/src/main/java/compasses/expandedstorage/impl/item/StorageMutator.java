...
public final class StorageMutator extends Item implements EntityInteractableItem {
    ...
    public static MutationMode getMode(ItemStack stack) {
        if (!stack.has(DataComponents.CUSTOM_DATA)) {
            stack.set(DataComponents.CUSTOM_DATA, CustomData.EMPTY.update(tag -> tag.putByte("mode", (byte) 0)));
        }

        CustomData data = stack.get(DataComponents.CUSTOM_DATA);

        if (!data.contains("mode")) {
            data.update(tag -> tag.putByte("mode", (byte) 0));
        }

        return MutationMode.from(data.getUnsafe().getByte("mode"));
    }
